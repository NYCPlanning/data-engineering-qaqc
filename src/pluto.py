def pluto():
    ...
    def get_data():
        def convert(dt):
            try:
                ...
                return dt
        engine = create_engine(ENGINE)
        df_mismatch = pd.read_sql(
            'SELECT * FROM dcp_pluto.qaqc_mismatch', con=engine)
        df_null = pd.read_sql('SELECT * FROM dcp_pluto.qaqc_null', con=engine)
        df_aggregate = pd.read_sql(
            'SELECT * FROM dcp_pluto.qaqc_aggregate', con=engine)
    ...
    def create_mismatch(df_mismatch, v1, v2, v3, condo, mapped):
        ...
        v1v2 = df.loc[df_mismatch.pair ==
                      f'{v1} - {v2}', :].to_dict('records')[0]
        v2v3 = df.loc[df_mismatch.pair ==
                      f'{v2} - {v3}', :].to_dict('records')[0]
    ...
    def get_expected_value(v1, v2):
        engine = create_engine(ENGINE)
        df = pd.read_sql(
            f"SELECT * FROM dcp_pluto.qaqc_expected where v ~* '{v1}|{v2}'",
            con=engine)
        return df
