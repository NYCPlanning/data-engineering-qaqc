...
def cpdb():
    ...
    st.markdown(
        ...
        The QAQC page is designed to highlight key measures that can indicate potential data issues in a CPDB build. These graphs are aggregated at the agency level and there are essentially 3 cuts of the data that can be selected and viewed:

        1. Type of Agency: Sponsoring Agency OR Managing Agency 
        2. Focus on the Commitment (Total sum ($) of all commitments) level data OR Project (# of total projects) level data at the specified agency level
        3. Select by Project/Commitment Category Type: All Categories (fixed asset, lump sum, ITT, Vehicles & equpment) OR only Fixed Asset's
