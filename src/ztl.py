def ztl(): 
    ...
    def get_data():
        ...
        qc_frequencychanges['percent'] = (qc_frequencychanges['countnew']/qc_frequencychanges['countold']-1)*100
        ...
        qc_versioncomparisonnownullcount['percent'] = (qc_versioncomparisonnownullcount['newnullcount']/qc_versioncomparisonnownullcount['oldnullcount']-1)*100

        return source_data_versions, bbldiff, last_build, \
                qc_frequencychanges, qc_versioncomparison, \
                qc_versioncomparisonnownullcount, \
                qaqc_mismatch, qaqc_frequency, qaqc_bbl
    
    source_data_versions, bbldiff, last_build, \
    qc_frequencychanges, qc_versioncomparison, \
    qc_versioncomparisonnownullcount, \
    qaqc_mismatch, qaqc_frequency, qaqc_bbl= get_data()
    ...
    meta = {
        ...
        'Zoning Districts':['zoningdistrict1', 'zoningdistrict2', 'zoningdistrict3','zoningdistrict4'], 
        'Special Districts':['specialdistrict1', 'specialdistrict2', 'specialdistrict3'], 
        'Other':['zoningmapcode', 'zoningmapnumber', 'limitedheightdistrict'], 
        'All':['commercialoverlay1', 'commercialoverlay2','zoningdistrict1', 
                'zoningdistrict2', 'zoningdistrict3','zoningdistrict4',
                'specialdistrict1', 'specialdistrict2', 'specialdistrict3',
                'zoningmapcode', 'zoningmapnumber', 'limitedheightdistrict']
    }
    lookup = {
    ...
    group = st.selectbox('select column group:', list(meta.keys()), index=len(meta.keys())-1)
    if group != 'All':
    ...
    if group == 'All':
        ...
        col = lookup[column]
        df=bbldiff.loc[bbldiff[f'{col}new'] != bbldiff[f'{col}prev'], ['bblnew', f'{col}new', f'{col}prev']]
    ...
    st.dataframe(qc_frequencychanges\
                    .loc[qc_frequencychanges.field.isin(meta[group])]\
    ...
    st.dataframe(qc_versioncomparison\
                    .loc[qc_versioncomparison.field.isin(meta[group])]\
                    .style.format({'percent': '{:,.2f}%'.format}))
    st.markdown('''
    ...
    st.dataframe(qc_versioncomparisonnownullcount\
                    .loc[qc_versioncomparisonnownullcount.field.isin(meta[group])]\
                    .style.format({'percent': '{:,.2f}%'.format}))
    st.markdown('''
